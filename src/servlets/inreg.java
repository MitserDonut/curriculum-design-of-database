 ...
 */
@WebServlet("/inreg")
public class inreg extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String sql="insert into _In(Gno,Wno,Sno,Ano,Idate,Inum,Spno,Iaddr,Ibz) values("
				+ "?,?,?,?,?,?,?,?,?)";
		try {
			...
			prst.setString(5, request.getParameter("year")+"-"+request.getParameter("month")+"-"+request.getParameter("date"));
			prst.setInt(6,Integer.parseInt(request.getParameter("num")));
			prst.setInt(7,Integer.parseInt(request.getParameter("Spno")));
			prst.setString(8, request.getParameter("address"));
			prst.setString(9, request.getParameter("bz"));
