...
public class supplierreg extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String sql="insert into _Supplier(Spname,Ssex,Scall,Sptype,Spbz) values("
				+ "?,?,?,?,?)";
		String sql1="update _Supplier set Spname=?,Ssex=?,Scall=?,Sptype=?,Spbz=? where Spno=?";
		try {
			if("reg".equals(action)){
				prst=conn.prepareStatement(sql);
				}
				else if(action.equals("modify")){
					no=(int) hs.getAttribute("mdfsid");
