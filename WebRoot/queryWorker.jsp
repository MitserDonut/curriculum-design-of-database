...
<html>
  <head>
	...
	if(pass!=null){
		response.sendRedirect("Doquery?type=q_workerInfo");
	}
...
<div class="show sss">
	<table>
		...
		System.out.println(i);%>
			<tr>
				<td><%=inv.get(i).getWk_no() %></td>
				<td><%=inv.get(i).getWk_name() %></td>
				<td><%=inv.get(i).getWk_sex() %></td>
				<td><%=inv.get(i).getWk_tete() %></td>
				<td><a href="modifyworker.jsp?row=<%=i %>">修改</a></td>
			</tr>
			<%} 
			}%>
			</tbody>
