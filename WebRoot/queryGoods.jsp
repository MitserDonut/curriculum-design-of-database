...
<html>
  <head>
	...
	<%
	Vector<GoodsInfo> inv=null;
	int power=99;
	String pass;
	pass=(String)request.getAttribute("pass");
	if(pass!=null){
		response.sendRedirect("Doquery?type=q_goodsList");
	}
	...
	<%
	if(ms.getAttribute("goodsInfo")!=null){
	inv=(Vector<GoodsInfo>)ms.getAttribute("goodsInfo");
	}
	else{
	response.sendRedirect("login.jsp");
	}
...
<div class="show sss">
	<table>
		...
		System.out.println(i);%>
			<tr>
				<td><%=inv.get(i).getGoods_no() %></td>
				<td><%=inv.get(i).getGoods_name() %></td>
				<td><%=inv.get(i).getGoods_fresh() %></td>
				<td><%=inv.get(i).getGoods_factory() %></td>
				<td><%=inv.get(i).getGoods_price() %></td>
			</tr>
			<%} 
			}%>
			</tbody>
