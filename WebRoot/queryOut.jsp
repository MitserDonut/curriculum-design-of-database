...
<html>
  <head>
	...
	<%
	Vector<IORecord> inv=null;
	int power=99;
	String pass;
	pass=(String)request.getAttribute("pass");
	if(pass!=null){
		response.sendRedirect("Doquery?type=q_outRecord");
	}
	...
	<%
	if(ms.getAttribute("outRecord")!=null){
	inv=(Vector<IORecord>)ms.getAttribute("outRecord");
	}
	else{
	response.sendRedirect("login.jsp");
	}
	...
	<table>
		...
		System.out.println(i);%>
			<tr>
				...
				<td><%=inv.get(i).getAdrs() %>
				<td><%=inv.get(i).getRole() %>
				<td><%=inv.get(i).getRole_name() %>
