...
<html>
  <head>
	...
	<%
	Vector<Inventory> inv=null;
	int power=99;
	String pass;
	pass=(String)request.getAttribute("pass");
	if(pass!=null){
		response.sendRedirect("Doquery?q_inven");
	}
	...
	<%
	if(ms.getAttribute("Inventory")!=null){
	inv=(Vector<Inventory>)ms.getAttribute("Inventory");
	}
	else{
	response.sendRedirect("login.jsp");
	}
	...
	<table>
		...
		System.out.println(i);%>
			<tr>
				...
				<td><%=inv.get(i).getAmount() %></td>
				<td><%=inv.get(i).getTotal() %></td>
			</tr>
			<%} 
			}%>
			</tbody>
