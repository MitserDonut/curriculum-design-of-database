...
<html>
  <head>
	...
	if(pass!=null){
		response.sendRedirect("Doquery?q_clientInfo");
	}
...
<div class="show sss">
	<table>
		...
		System.out.println(i);%>
			<tr>
				<td><%=inv.get(i).getClt_no() %></td>
				<td><%=inv.get(i).getClt_name() %></td>
				<td><%=inv.get(i).getClt_type() %></td>
				<td><%=inv.get(i).getClt_tele() %></td>
				<td><%=inv.get(i).getClt_sex() %></td>
				<td><%=inv.get(i).getClt_bz() %></td>
				<td><a href="modifyclient.jsp?row=<%=i %>">修改</a></td>
			</tr>
			<%} 
			}%>
			</tbody>
