...
<html>
  <head>
	...
	if(pass!=null){
		response.sendRedirect("Doquery?q_adminInfo");
	}
	...
	<%
	if(ms.getAttribute("adminInfo")!=null){
	inv=(Vector<AdminInfo>)ms.getAttribute("adminInfo");
	}
	else{
	response.sendRedirect("login.jsp");
	} %>
	...
	<table>
		...
		System.out.println(i);%>
			<tr>
				...
				<td><%=inv.get(i).getAd_sex() %></td>
				<td><%=inv.get(i).getAd_tele() %></td>
				<td><%=inv.get(i).getAd_belong() %></td>
				<td><%=inv.get(i).getAd_bname() %></td>
				<td><a href=<%="modifyadmin.jsp?row="+String.valueOf(i) %>>>修改</a></td>
			</tr>
			<%} 
			}%>
			</tbody>
