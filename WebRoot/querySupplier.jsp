...
<html>
  <head>
	...
	if(pass!=null){
		response.sendRedirect("Doquery?q_supplierInfo");
	}
	...
	inv=(Vector)ms.getAttribute("supplierInfo");
	}
	else{
	response.sendRedirect("login.jsp");
	}
...
<div class="show sss">
	<table>
		...
		System.out.println(i);%>
			<tr>
				<td><%=inv.get(i).getSup_no() %></td>
				<td><%=inv.get(i).getSup_name() %></td>
				<td><%=inv.get(i).getSup_type() %></td>
				<td><%=inv.get(i).getSup_tele() %></td>
				<td><%=inv.get(i).getSup_sex() %></td>
				<td><%=inv.get(i).getSup_bz() %></td>
				<td><a href="modifysupplier.jsp?row=<%=i %>">修改</a></td>
			</tr>
			<%} 
			}%>
			</tbody>
